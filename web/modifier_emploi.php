<?php
/* Permet la modification des emplois après mise en ligne sur le réseau */
$database = "likedin";
session_start();
...
if ($db_found) {
  ...
  echo '<form enctype="multipart/form-data" method="POST"><div class="row g-0">
    <div class="col-sm-6 col-md-8"><br>
      <h3>Titre de l\'emploi :</h3><h4> <input type="text" class="form-control" maxlength="30" style="width: 700px;" name="Nom" value="'.$data['Nom'].'" required/></h4><br><br>
  ...
  if(array_key_exists('soumettre',$_POST)){
    ...
    if($uploadfile != $uploaddir){
      ...
      $sql='UPDATE `emploi` SET Nom = "'.$Nom.'", Desc_courte = "'.$Desc_courte.'", Description = "'.$Description.'", Image = "'.$uploadfile.'" WHERE ID_Emploi = "'.$ID_Emploi.'"';
    }
    else{$sql='UPDATE `emploi` SET Nom = "'.$Nom.'", Desc_courte = "'.$Desc_courte.'", Description = "'.$Description.'" WHERE ID_Emploi = "'.$ID_Emploi.'"';}
    ...
    header("Location: admin_menu_emploi.php");
